...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`CREATE INDEX \`_posts_versioned_v_snapshot_idx\` ON \`_posts_versioned_v\` (\`snapshot\`);`,
  )
  await db.run(
  ...
  await db.run(
    sql`CREATE INDEX \`_global_versioned_v_snapshot_idx\` ON \`_global_versioned_v\` (\`snapshot\`);`,
  )
  await db.run(
